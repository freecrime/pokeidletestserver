...
function loadSaveSlotFromPicker(slotIdx) {
  ...
  try {
    ...
    if(gameState.road && gameState.road.mode === 'ascended') { gameState.road.active = false; gameState.road.mode = null; }
    ...
    if(typeof pfMigrateRewardSprites === 'function') pfMigrateRewardSprites();
...
async function spawnEnemy() {
  if(bossStarting || bossBattleActive || dialgaBattleActive || palkiaBattleActive || arceusRaidActive || giratinaBattleActive || kyuremBattleActive || zygardeBattleActive || swordShieldBattleActive || eventBossBattleActive || vrBattleActive || mcBattleActive) return;
...
function battleTick() {
  ...
  // ── Origin Dialga + Origin Palkia team synergy (computes once; runs its own strike + visuals) ──
  const syn = tickOriginSynergy(player, now);
  if(!currentEnemy) return; // Palkia's rift strike may have finished the enemy
  ...
  let playerDelay = isDeoxysSpeed
    ? Math.max(450, BASE_ATTACK_MS - playerSpeed * 5) // Speed form: floor 450ms
    : Math.max(800, BASE_ATTACK_MS - playerSpeed * 5);
  if(syn.haste) playerDelay = Math.floor(playerDelay / 1.5); // Dialga accelerates time → Palkia attacks 1.5× faster
  ...
  let enemyDelay = currentEnemy._attackInterval || Math.max(900, BASE_ATTACK_MS - enemySpeed * 5);
  if(syn.slow) enemyDelay = Math.floor(enemyDelay * 1.15); // Origin Dialga warps time → foe attacks 15% slower
...
function doPlayerAttack(player) {
  ...
  showDmgNum(dmg, 'enemy-area', isCrit ? '#ffd54f' : dmgColor, { crit: isCrit });
  spawnImpact('enemy-sprite', { crit: isCrit, color: isCrit ? '#ffd54f' : '#ffb74d' });
  animateHurt('enemy-sprite');
  if(isCrit) arenaShake();
  addLog(`${player.name} dealt ${dmg} damage${isCrit ? ' — CRITICAL HIT!' : '!'}`, 'log-dmg');
...
function doEnemyAttack(player) {
  ...
  spawnImpact('player-sprite', { color: '#ff6b6b' });
...
// ── Origin Dialga + Origin Palkia team synergy ──────────────────────────────
// While Origin Dialga is the ACTIVE fighter and an Origin Palkia is also on the
// team, Dialga warps time (foe 15% slower, applied in battleTick) and Palkia
// strikes the foe from the rift every 5s. Both are shown via the arena banner,
// a "time-slowed" aura on the foe, and a spatial-rift burst on each strike.
let _palkiaStrikeCD = 0;
function tickOriginSynergy(activePlayer, now) {
  const res = { slow: false, haste: false };
  const dialgaActive = activePlayer && typeof isOriginDialga === 'function' && isOriginDialga(activePlayer);
  const palkiaActive = activePlayer && typeof isOriginPalkia === 'function' && isOriginPalkia(activePlayer);
  // Need the OTHER Origin forme alive in the back row.
  let partner = null;
  if((dialgaActive || palkiaActive) && gameState.team) {
    for(let i = 0; i < gameState.team.length; i++) {
      const p = gameState.team[i];
      if(p && p.uid !== activePlayer.uid && p.currentHp > 0 &&
         (dialgaActive ? isOriginPalkia(p) : isOriginDialga(p))) { partner = p; break; }
    }
  }

  if(dialgaActive && partner) {
    // Origin Dialga out, Origin Palkia in back row → slow the foe 15% + Palkia rift-strikes every 3.5s
    res.slow = true;
    setSynergyVisual('dialga');
    if(currentEnemy) {
      if(!_palkiaStrikeCD) _palkiaStrikeCD = now + 3500;
      if(now >= _palkiaStrikeCD) { _palkiaStrikeCD = now + 3500; originPalkiaStrike(partner, activePlayer); }
    } else { _palkiaStrikeCD = 0; }
  } else if(palkiaActive && partner) {
    // Origin Palkia out, Origin Dialga in back row → Dialga accelerates time, Palkia attacks 1.5× faster
    res.haste = true;
    setSynergyVisual('palkia');
    _palkiaStrikeCD = 0;
  } else {
    setSynergyVisual(null);
    _palkiaStrikeCD = 0;
  }
  return res;
}

function originPalkiaStrike(palkia, activePlayer) {
  if(!currentEnemy || currentEnemy.currentHp <= 0 || !palkia) return;
  let dmg = Math.max(1, Math.floor(getAttack(palkia) * 0.6)); // back-row strike: ~60% of Origin Palkia's attack
  const crit = Math.random() < 0.12;
  if(crit) dmg = Math.floor(dmg * 1.5);
  currentEnemy.currentHp = Math.max(0, currentEnemy.currentHp - dmg);
  spawnPalkiaRift();
  showDmgNum(dmg, 'enemy-area', '#e79bd9', { crit });
  spawnImpact('enemy-sprite', { color: '#e79bd9', crit });
  addLog(`🌀 Origin Palkia struck from the rift for ${dmg}${crit ? ' — CRITICAL!' : ''}!`, 'log-cosmic');
  updateEnemyUI();
  if(currentEnemy.currentHp <= 0) enemyDefeated(activePlayer);
}

function spawnPalkiaRift() {
  if(window._animationsDisabled) return;
  const enemy = document.getElementById('enemy-sprite');
  const arena = document.getElementById('arena');
  if(!enemy || !arena) return;
  const sr = enemy.getBoundingClientRect();
  const ar = arena.getBoundingClientRect();
  if(sr.width === 0) return;
  const cx = sr.left - ar.left + sr.width / 2;
  const cy = sr.top - ar.top + sr.height / 2;
  const rift = document.createElement('div');
  rift.className = 'palkia-rift';
  rift.style.left = cx + 'px';
  rift.style.top = cy + 'px';
  arena.appendChild(rift);
  setTimeout(() => rift.remove(), 680);
}

function setSynergyVisual(mode) {
  const on = mode === 'dialga' || mode === 'palkia';
  const ind = document.getElementById('synergy-indicator');
  if(ind) {
    if(ind.classList.contains('active') !== on) ind.classList.toggle('active', on);
    if(on) {
      const txt = ind.querySelector('.syn-text');
      const html = mode === 'dialga'
        ? '<b>SPACETIME SYNERGY</b> · Dialga slows the foe&nbsp;·&nbsp;Palkia strikes from the rift'
        : '<b>SPACETIME SYNERGY</b> · Dialga accelerates time&nbsp;·&nbsp;Palkia attacks 1.5× faster';
      if(txt && txt.innerHTML !== html) txt.innerHTML = html;
    }
  }
  // Foe time-slow aura only in Dialga mode; player haste aura only in Palkia mode.
  const ea = document.getElementById('enemy-area');     // toggle on AREAs (sprite class is re-set each tick)
  const slow = mode === 'dialga';
  if(ea && ea.classList.contains('time-slowed') !== slow) ea.classList.toggle('time-slowed', slow);
  const pa = document.getElementById('player-area');
  const haste = mode === 'palkia';
  if(pa && pa.classList.contains('time-hasted') !== haste) pa.classList.toggle('time-hasted', haste);
}

function enemyDefeated(player) {
  if(currentEnemy && currentEnemy._ascended) { handleAscendedStageClear(player); return; }
  if(bossBattleActive && currentEnemy?.isBoss) {
    ...
    else if(currentEnemy?._isDialga) handleDialgaDefeated(player);
    else if(currentEnemy?._isPalkia) handlePalkiaDefeated(player);
    else if(currentEnemy?._isArceus) handleArceusDefeated(player);
...
function playerPokemonFainted(player) {
  ...
  if(bossBattleActive && currentEnemy?.isBoss) {
    ...
    if(nextIdx >= 0) {
      ...
      updatePlayerUI(next);
      playerAttackCooldown = 0;
      enemyAttackCooldown = Date.now() + (currentEnemy?._attackInterval ? 1300 : 800);
      renderBottomBar();
    } else {
      if(ascendedRunActive) handleAscendedFailed();
      else if(vrBattleActive) handleVRFailed();
      ...
      else if(dialgaBattleActive) handleDialgaFailed();
      else if(palkiaBattleActive) handlePalkiaFailed();
      else if(arceusRaidActive) handleArceusFailed();
...
// ████████████████████████████████████████████████████████████
// ASCENDED PLANE — a 50-stage, no-revive boss gauntlet
//   • Entry: 250 💎   • Clear all 50 → 450 💎 + a gamble round
//   • Your party does NOT auto-revive: faint all 6 and the run ends.
//   • Enemies are fixed endgame bosses (stats-less, so getAttack→50
//     and we scale damage purely with _attackMult, HP with _bossHpMax).
//   • TUNING KNOBS live in spawnAscendedEnemy (ascHp / ascDmg).
// ████████████████████████████████████████████████████████████
let ascendedRunActive = false;
const ASCENDED_STAGES = 50;
// Boss-flavoured roster the gauntlet pulls sprites/names from (purely cosmetic).
const ASCENDED_POOL = [
...
function enterAscendedPlane() {
  if(gameState.road.active || bossBattleActive) { toast('Finish your current battle first!', 3000); return; }
  if((gameState.team || []).length < 1) { toast('You need a team first!', 3000); return; }
  if(gameState.gems < 250) { toast('Need 250 💎 to enter the Ascended Plane!', 3000); return; }
  gameState.gems -= 250;
  updateResourceUI();
  // Fresh start — full heal once, then NO revives for the rest of the run.
  gameState.team.forEach(p => { if(p && p.statsLoaded) p.currentHp = getMaxHp(p); });
  gameState.currentFighterIdx = 0;
  gameState.road.active = true;
  gameState.road.mode = 'ascended';
  gameState.road.ascStage = 1;
  ascendedRunActive = true;
  bossBattleActive = true;
  document.getElementById('arena').classList.add('road-mode');
  addLog('🌌 Entered the ASCENDED PLANE — 50 stages, NO revives. Survive!', 'log-cosmic');
  toast('🌌 Ascended Plane — Stage 1/50', 3000);
  if(typeof renderRoadUI === 'function') renderRoadUI();
  if(typeof switchTab === 'function') switchTab('team');
  spawnAscendedEnemy();
  updateRoadProgressHUD();
}

function spawnAscendedEnemy() {
  const stage = gameState.road.ascStage || 1;
  const pick = ASCENDED_POOL[Math.floor(Math.random() * ASCENDED_POOL.length)];
  // ── TUNING ── tuned so a full endgame team clears all 50 comfortably (no
  // revives — but surviving mons "rest"/heal between stages, see below).
  // Raise these two if you want it harder; lower them if it's still too tough.
  const ascHp  = Math.floor(8000 + stage * 600);    // stage 1 ≈ 8.6k → stage 50 ≈ 38k
  const ascDmg = Math.floor(110 + stage * 6);        // per-hit ≈ 116 → ≈ 410 (before your defensive items)
  const enemy = {
    uid: 'asc_' + stage,
    id: pick.id, name: pick.name, types: pick.types,
    level: Math.min(290, 250 + stage), isShiny: false,
    stats: null, statsLoaded: true,            // stats-less ⇒ getAttack/getSpeed fall back to 50
    isBoss: true, _ascended: true,
    currentHp: ascHp, _bossHpMax: ascHp,        // honoured by the enemy HP bar
    _attackInterval: 1150,                      // fixed swing speed
    _attackMult: ascDmg / 50,                   // 50 (flat getAttack) × mult = ascDmg
  };
  currentEnemy = enemy;
  bossBattleActive = true;
  playerAttackCooldown = 0;
  enemyAttackCooldown = Date.now() + 900;       // your fresh mon swings first
  battleTick.lastRegen = 0;
  const flash = document.getElementById('encounter-flash');
  if(flash) { flash.style.transition = 'opacity 0.05s'; flash.style.opacity = '0.7'; setTimeout(()=>{ flash.style.transition = 'opacity 1.2s'; flash.style.opacity = '0'; }, 120); }
  updateEnemyUI();
  addLog(`🌌 Stage ${stage}/${ASCENDED_STAGES} — ${pick.name} (Lv.${enemy.level}) · ${formatNum(ascHp)} HP`, 'log-dmg');
}

function handleAscendedStageClear(player) {
  const stage = gameState.road.ascStage || 1;
  addLog(`✨ Stage ${stage} cleared!`, 'log-evolve');
  if(player) { const xp = 400 + stage * 40; giveExp(player, xp); }
  // "Rest" after each boss: surviving mons recover 50% — but fainted ones stay
  // down (no revive), so wiping all 6 still ends the run.
  gameState.team.forEach(p => {
    if(p && p.statsLoaded && p.currentHp > 0) p.currentHp = Math.min(getMaxHp(p), p.currentHp + Math.floor(getMaxHp(p) * 0.5));
  });
  currentEnemy = null;
  if(stage >= ASCENDED_STAGES) { handleAscendedComplete(); return; }
  gameState.road.ascStage = stage + 1;
  updateRoadProgressHUD();
  if(typeof renderRoadUI === 'function') renderRoadUI();
  setTimeout(() => { if(ascendedRunActive) spawnAscendedEnemy(); }, 900);
}

function handleAscendedComplete() {
  const reward = 450;
  ascendedRunActive = false;
  ...
  updateResourceUI();
  addLog(`👑 ASCENDED PLANE CONQUERED! All 50 stages cleared! +${reward} 💎`, 'log-cosmic');
  toast(`👑 You conquered the Ascended Plane! +${reward} 💎`, 5000);
  updateRoadProgressHUD();
  if(typeof renderRoadUI === 'function') renderRoadUI();
  if(typeof saveGame === 'function') saveGame();
  // Bonus gamble with the reward.
  if(typeof openAscendedGamble === 'function') setTimeout(() => openAscendedGamble(reward), 900);
  setTimeout(spawnEnemy, 1200);
}

function handleAscendedFailed() {
  ascendedRunActive = false;
  bossBattleActive = false;
  const reached = gameState.road.ascStage || 1;
  currentEnemy = null;
  gameState.road.active = false;
  gameState.road.mode = null;
  document.getElementById('arena').classList.remove('road-mode');
  // Revive the team at 25% so the player can keep playing normally.
  gameState.team.forEach(p => { if(p && p.statsLoaded) p.currentHp = Math.max(1, Math.floor(getMaxHp(p) * 0.25)); });
  gameState.currentFighterIdx = 0;
  addLog(`💀 Your team was wiped on Stage ${reached}/${ASCENDED_STAGES}. The Ascended Plane bested you.`, 'log-dmg');
  toast(`💀 Wiped on Stage ${reached}. Better luck next run!`, 4000);
  updateRoadProgressHUD();
  if(typeof renderRoadUI === 'function') renderRoadUI();
  if(typeof saveGame === 'function') saveGame();
...
function exitAscendedPlane() {
  if(!ascendedRunActive) return;
  const reached = gameState.road.ascStage || 1;
  ascendedRunActive = false;
  bossBattleActive = false;
  currentEnemy = null;
  ...
  gameState.team.forEach(p => { if(p && p.statsLoaded) p.currentHp = getMaxHp(p); });
  gameState.currentFighterIdx = 0;
  addLog(`🚪 Left the Ascended Plane at Stage ${reached}. No reward — run abandoned.`, 'log-heal');
  toast('Left the Ascended Plane.', 2500);
  updateRoadProgressHUD();
  if(typeof renderRoadUI === 'function') renderRoadUI();
...
function getPokemonSpriteClass(pokemon) {
  if(pokemon._achievementMon) return 'fighter-sprite achievement-sprite';
  if(pokemon._legend) return pokemon.isShiny ? 'fighter-sprite legendshiny-sprite' : 'fighter-sprite legend-sprite';
...
function updatePlayerUI(player) {
  ...
  const sprite = document.getElementById('player-sprite');
  const psrc = getBattleSprite(player.id, player.isShiny, player.uid);
  if (sprite.getAttribute('src') !== psrc && sprite.getAttribute('src') !== player._spriteFallback) {
    sprite.onerror = player._spriteFallback ? function(){ this.onerror=null; this.src=player._spriteFallback; } : null;
    sprite.src = psrc;
  }
  const pcls = getPokemonSpriteClass(player);
  if (sprite.className !== pcls) sprite.className = pcls;
  updateSpriteSparkles('player-sprite', player);
}
...
function updateEnemyUI() {
  ...
  if(currentEnemy._customSprite) {
    esrc = (currentEnemy.isShiny && currentEnemy._customSpriteShiny) ? currentEnemy._customSpriteShiny : currentEnemy._customSprite;
  } else {
    esrc = getBattleSprite(currentEnemy.id, currentEnemy.isShiny);
  }
  if (sprite.getAttribute('src') !== esrc && sprite.getAttribute('src') !== currentEnemy._spriteFallback) {
    sprite.onerror = currentEnemy._spriteFallback ? function(){ this.onerror=null; this.src=currentEnemy._spriteFallback; } : null;
    sprite.src = esrc;
  }
  const ecls = getPokemonSpriteClass(currentEnemy);
  if (sprite.className !== ecls) sprite.className = ecls;
  updateSpriteSparkles('enemy-sprite', currentEnemy);
}

// Overlay twinkling star sparkles on shiny / cosmic battle sprites.
// Rebuilds only when the Pokémon (or its tier) changes, so animations don't restart each tick.
function updateSpriteSparkles(spriteId, pokemon) {
  const sprite = document.getElementById(spriteId);
  if(!sprite) return;
  const parent = sprite.parentElement;
  if(!parent) return;
  let layer = parent.querySelector('.spark-layer');
  const off = !!(window._animationsDisabled || (window._gameOptions && window._gameOptions.performance));
  const legend = !!(pokemon && pokemon._legend);
  const cosmic = !!(pokemon && isCosmic(pokemon) && !legend);
  const shiny = !!(pokemon && pokemon.isShiny && !cosmic && !legend);
  const ach = !!(pokemon && pokemon._achievementMon && !legend && !cosmic && !shiny);
  if(off || (!legend && !cosmic && !shiny && !ach)) { if(layer) layer.remove(); return; }

  const tier = ach ? 'achievement' : legend ? (pokemon.isShiny ? 'legendshiny' : 'legend') : cosmic ? 'cosmic' : 'shiny';
  const key = (pokemon.uid || pokemon.id || '?') + ':' + tier;
  const place = () => {
    layer.style.left = sprite.offsetLeft + 'px';
    layer.style.top = sprite.offsetTop + 'px';
    layer.style.width = sprite.offsetWidth + 'px';
    layer.style.height = sprite.offsetHeight + 'px';
  };
  if(layer && layer.dataset.key === key) { place(); return; }
  if(layer) layer.remove();

  layer = document.createElement('div');
  layer.className = 'spark-layer spark-' + tier;
  layer.dataset.key = key;
  place();
  const n = tier === 'legendshiny' ? 12 : tier === 'legend' ? 10 : tier === 'achievement' ? 7 : cosmic ? 9 : 7;
  // Legendaries get a pulsing aura halo behind the sprite. Achievement mons use an
  // edge-only rim glow instead (the .achievement-sprite drop-shadow), so no centered halo.
  let html = legend ? '<div class="spark-aura"></div>' : '';
  for(let i = 0; i < n; i++) {
    const x = ach ? (7 + Math.random() * 86).toFixed(1) : (24 + Math.random() * 52).toFixed(1);   // achievement: whole sprite
    const y = ach ? (7 + Math.random() * 86).toFixed(1) : (22 + Math.random() * 54).toFixed(1);
    const sz = Math.round((legend ? 17 : ach ? 16 : cosmic ? 16 : 14) + Math.random() * 12);
    const dur = (ach ? 2.6 + Math.random() * 1.6 : 1.2 + Math.random() * 1.2).toFixed(2);
    const dly = (Math.random() * (ach ? 3.5 : 2)).toFixed(2);
    html += `<i class="spark-star" style="left:${x}%;top:${y}%;width:${sz}px;height:${sz}px;--dur:${dur}s;--dly:${dly}s"></i>`;
  }
  ...
  for(let i=0; i<6; i++) {
    ...
    if(p) {
      ...
      div.innerHTML = `
        ...
        <img src="${getSpriteUrl(p.id, p.isShiny, p.uid)}" onerror="this.onerror=null;this.src='${p._spriteFallback || getBattleSprite(p.id, p.isShiny, p.uid)}'">
...
function renderPokemonBox() {
  ...
  sorted.forEach((p) => {
    ...
    div.className = 'poke-card' + (p._achievementMon ? ' achievement-card' : isMegaRayquaza(p) ? ' mega-card' : (isOriginGiratina(p) || isOriginDialga(p) || isOriginPalkia(p)) ? ' origin-card' : isDNAFused(p) ? ' mega-card' : isCrownedZacian(p) || isCrownedZamazenta(p) ? ' origin-card' : cosmic ? ' cosmic-card' : p.isShiny ? ' shiny-card' : '');
    ...
    div.innerHTML = `
      ${p._achievementMon ? '<div style="position:absolute;top:3px;right:4px;font-size:13px;filter:drop-shadow(0 0 4px rgba(255,215,0,0.95))" title="Achievement reward">🏅</div>' : cosmic ? '<div style="position:absolute;top:3px;right:4px;font-size:12px">🌌</div>' : p.isShiny ? '<div style="position:absolute;top:3px;right:4px;font-size:14px">★</div>' : ''}
...
function renderItems() {
  ...
  allItems.forEach(item => {
    ...
    if(item.id === 'max_candy') { div.style.border = '1px solid #ff69b4'; div.style.boxShadow = '0 0 8px rgba(255,105,180,.45)'; }
    else if(item.id === 'sss_candy') { div.style.border = '1px solid #ffd700'; div.style.boxShadow = '0 0 8px rgba(255,215,0,.40)'; }
    else if(isEpic) { div.style.border = '1px solid #e040fb'; div.style.boxShadow = '0 0 7px rgba(224,64,251,.35)'; }
    div.innerHTML = `${item.emoji}<span class="item-count">×${count}</span>`;
    div.onclick = () => showItemModal(item);
    div.oncontextmenu = (e) => { e.preventDefault(); trashItem(item.id); };
    grid.appendChild(div);
...
function animateAttack(who) {
  ...
  if(!sprite || !sprite.animate) return;
  // Run the lunge via the Web Animations API (transform only) so it never touches the
  // sprite's CSS `animation` (the glow) — the aura keeps pulsing/cycling uninterrupted.
  const kf = isPlayer ? [
    { transform: 'scaleX(-1) translate(0px,0px) rotate(0deg)',      offset: 0 },
    { transform: 'scaleX(-1) translate(-16px,-12px) rotate(-4deg)', offset: 0.2 },
    { transform: 'scaleX(-1) translate(-90px,0px) rotate(2deg)',    offset: 0.45 },
    { transform: 'scaleX(-1) translate(-38px,-6px) rotate(-1deg)',  offset: 0.7 },
    { transform: 'scaleX(-1) translate(0px,0px) rotate(0deg)',      offset: 1 },
  ] : [
    { transform: 'translate(0px,0px) rotate(0deg)',     offset: 0 },
    { transform: 'translate(-16px,-12px) rotate(4deg)', offset: 0.2 },
    { transform: 'translate(-90px,0px) rotate(-2deg)',  offset: 0.45 },
    { transform: 'translate(-38px,-6px) rotate(1deg)',  offset: 0.7 },
    { transform: 'translate(0px,0px) rotate(0deg)',     offset: 1 },
  ];
  sprite.animate(kf, { duration: 500, easing: 'cubic-bezier(0.25,0.46,0.45,0.94)' });
}
...
function animateHurt(id) {
  ...
  if(!sprite || !sprite.animate) return;
  const isPlayer = id === 'player-sprite';
  const f = isPlayer ? 'scaleX(-1) ' : '';
  // Capture the live glow so it's preserved between flash frames (auto-handles every
  // tier — shiny/cosmic/legend/achievement). WAAPI overrides the filter only for the
  // flash, then releases; the underlying CSS glow animation keeps running and resumes seamlessly.
  let glow = 'none';
  try { const cs = getComputedStyle(sprite).filter; if (cs && cs !== 'none') glow = cs; } catch(e){}
  const flash  = 'brightness(4) sepia(1) hue-rotate(300deg)';
  const flash2 = 'brightness(3) sepia(1) hue-rotate(300deg)';
  sprite.animate([
    { filter: glow,   transform: f + 'translateX(0)',    offset: 0 },
    { filter: flash,  transform: f + 'translateX(-12px)', offset: 0.15 },
    { filter: glow,   transform: f + 'translateX(10px)',  offset: 0.30 },
    { filter: flash2, transform: f + 'translateX(-8px)',  offset: 0.50 },
    { filter: glow,   transform: f + 'translateX(6px)',   offset: 0.70 },
    { filter: glow,   transform: f + 'translateX(0)',     offset: 1 },
  ], { duration: 450, easing: 'ease-out' });
}

function showDmgNum(dmg, areaId, color, opts) {
  if(window._gameOptions && window._gameOptions.hideDmgNumbers) return;
  opts = opts || {};
  const area = document.getElementById(areaId);
  if(!area) return;
  const div = document.createElement('div');
  div.className = 'dmg-num' + (opts.crit ? ' crit' : '') + (opts.heal ? ' heal' : '');
  div.textContent = (opts.heal ? '+' : '-') + dmg;
...
// Burst of light + sparks at the point of impact when an attack lands.
function spawnImpact(spriteId, opts) {
  if(window._animationsDisabled) return;
  opts = opts || {};
  const sprite = document.getElementById(spriteId);
  const arena = document.getElementById('arena');
  if(!sprite || !arena) return;
  const sr = sprite.getBoundingClientRect();
  const ar = arena.getBoundingClientRect();
  if(sr.width === 0) return; // sprite not visible
  const cx = sr.left - ar.left + sr.width / 2;
  const cy = sr.top - ar.top + sr.height / 2;
  const wrap = document.createElement('div');
  wrap.className = 'impact' + (opts.crit ? ' crit' : '');
  wrap.style.left = cx + 'px';
  wrap.style.top = cy + 'px';
  wrap.style.setProperty('--ic', opts.color || '#ffb74d');
  let spk = '';
  const n = opts.crit ? 8 : 6;
  for(let i = 0; i < n; i++) spk += `<i class="spk" style="--a:${Math.round((360 / n) * i + (Math.random() * 30 - 15))}deg;animation-delay:${(Math.random() * 40).toFixed(0)}ms"></i>`;
  wrap.innerHTML = `<span class="core"></span><span class="ring"></span>${spk}`;
  arena.appendChild(wrap);
  setTimeout(() => wrap.remove(), 480);
}

// Quick screen shake for big moments (crits, boss hits).
function arenaShake(strong) {
  if(window._animationsDisabled) return;
  const arena = document.getElementById('arena');
  if(!arena) return;
  const cls = strong ? 'arena-shake-strong' : 'arena-shake';
  arena.classList.remove('arena-shake', 'arena-shake-strong');
  void arena.offsetWidth;
  arena.classList.add(cls);
  setTimeout(() => arena.classList.remove(cls), 360);
}
...
function addLog(msg, cls='') {
  if(cls === 'log-dmg' && window._gameOptions && window._gameOptions.quietLog) return;
...
async function doEpicGacha(count) {
  ...
  if(count === 1 && pokePulls.length === 1) showGachaResult(pokePulls[0], { again: () => doEpicGacha(1), label: '🔁 Pull Again (50💎)' });
  else if(count === 1 && pokePulls.length === 0) { /* item only */ renderAll(); }
  else showGachaResultMulti(pokePulls, { again: () => doEpicGacha(10), label: '🔁 Pull 10× Again (450💎)' });
  renderAll();
}
...
async function doPokemonGacha(count) {
  ...
  if(count === 1) showGachaResult(pulls[0], { again: () => doPokemonGacha(1), label: '🔁 Pull Again (5💎)' });
  else showGachaResultMulti(pulls, { again: () => doPokemonGacha(10), label: '🔁 Pull 10× Again (40💎)' });
  renderAll();
}
...
async function doItemGacha(count) {
  ...
  renderAll();
  if(count === 1) showItemGachaResult(gotten[0], { again: () => doItemGacha(1), label: '🔁 Pull Again (200💰)' });
  else showItemGachaResultMulti(gotten, { again: () => doItemGacha(10), label: '🔁 Pull 10× Again (1800💰)' });
}
...
async function doEpicItemGacha(count) {
  ...
  renderAll();
  if(count === 1) showItemGachaResult(gotten[0], { again: () => doEpicItemGacha(1), label: '🔁 Pull Again (5000💰)' });
  else showItemGachaResultMulti(gotten, { again: () => doEpicItemGacha(10), label: '🔁 Pull 10× Again (40000💰)' });
}
...
async function doPremiumItemGacha() {
  ...
  showItemGachaResultMulti(gotten, { again: () => doPremiumItemGacha(), label: '🔁 Premium Pull Again (1.5M💰)' });
}

// ── Limited Legendary Gacha ──────────────────────────────────────────────────
// Fixed event window — ends 10 June 2026, 23:59:59 (CEST, UTC+2).
const LIMITED_GACHA_END = new Date('2026-06-10T23:59:59+02:00').getTime();

function getLimitedGachaMs() {
  const left = LIMITED_GACHA_END - Date.now();
...
function updateLimitedGachaUI() {
  ...
  if(ms === null) {
    ...
    timerEl.textContent = 'Ends in: ' + (d > 0 ? d + 'd ' : '') + h + 'h ' + m + 'm ' + s + 's';
    timerEl.style.color = ms < 86400000 ? '#ff4444' : '#ffd54f';
    if(btnWrap) { btnWrap.style.opacity = '1'; btnWrap.style.pointerEvents = 'auto'; }
  }
}
setInterval(updateLimitedGachaUI, 1000);
setTimeout(updateLimitedGachaUI, 500);

// The four limited editions. The two "grand" forms use custom sprites.
const LIMITED_LEGENDS = {
  rare: [
    { id: 890, name: 'Eternatus', types: ['poison','dragon'] },
    { id: 892, name: 'Urshifu',   types: ['fighting','dark'] },
  ],
  grand: [
    { id: 890, name: 'Eternamax Eternatus', types: ['poison','dragon'],
      sprite:      'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/10190.png',
      shinySprite: 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/shiny/10190.png' },
    { id: 892, name: 'Single Strike Urshifu', types: ['fighting','dark'],
      sprite:      'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/10226.png',
      shinySprite: 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/shiny/10226.png' },
  ],
};

async function makeLimitedLegend(def, isGrand) {
  const isShiny = Math.random() < 1 / 5;   // each of the 4 has a 1-in-5 shiny chance
  const level = isGrand ? 250 : 200;       // high level befitting a super-limited legendary
  const pk = newPokemonEntry(def.id, def.name, def.types, level, isShiny, true);
  pk.stats = await fetchPokemonStats(def.id);
  // Boost base stats so they punch at their real weight class (grand forms hit even harder).
  if(pk.stats) {
    const mult = isGrand ? 1.35 : 1.15;
    pk.stats = pk.stats.map(s => ({ ...s, base_stat: Math.round(s.base_stat * mult) }));
    // Attack with whichever offense is stronger (Eternatus = special, Urshifu = physical).
    const atkB = pk.stats.find(s => s.stat.name === 'attack').base_stat;
    const spaB = pk.stats.find(s => s.stat.name === 'special-attack').base_stat;
    pk._attackMode = spaB > atkB ? 'special' : 'physical';
  }
  pk.ivs = { hp:31, attack:31, defense:31, 'special-attack':31, 'special-defense':31, speed:31 }; // perfect IVs
  pk._naturalSSS = true;                    // ×2 to HP and every combat stat (top tier)
  pk.statsLoaded = true;
  pk.currentHp = getMaxHp(pk);
  pk._legend = true;
  pk._gachaTier = isShiny ? 'legendshiny' : 'legend';
  if(def.sprite) pk._customSprite = (isShiny && def.shinySprite) ? def.shinySprite : def.sprite;
  return pk;
}

async function doLimitedLegendaryGacha(count) {
  if(getLimitedGachaMs() === null) { toast('The Limited Legendary Gacha has ended!'); return; }
  const cost = count === 1 ? 700 : 6500;
  if(gameState.gems < cost) { toast(`Need ${cost} 💎 Gems!`); return; }
  gameState.gems -= cost;
  updateResourceUI();

  const pulls = [];
  // Debug: force every pull to be a random one of the 4 limited editions (for testing animations).
  const debugAllLegends = window.debugGuaranteeLegendPull
    ? LIMITED_LEGENDS.rare.map(d => ({ def:d, grand:false })).concat(LIMITED_LEGENDS.grand.map(d => ({ def:d, grand:true })))
    : null;
  for(let i = 0; i < count; i++) {
    const roll = Math.random();
    let pk;
    if(debugAllLegends) {            // DEBUG ONLY — guaranteed random legendary
      const pick = debugAllLegends[Math.floor(Math.random() * debugAllLegends.length)];
      pk = await makeLimitedLegend(pick.def, pick.grand);
    } else if(roll < 0.001) {        // 0.1% — grand prize edition (special versions)
      pk = await makeLimitedLegend(LIMITED_LEGENDS.grand[Math.floor(Math.random() * LIMITED_LEGENDS.grand.length)], true);
    } else if(roll < 0.011) {        // +1% — Eternatus / Urshifu
      pk = await makeLimitedLegend(LIMITED_LEGENDS.rare[Math.floor(Math.random() * LIMITED_LEGENDS.rare.length)], false);
    } else {                          // otherwise: same as the normal Epic Pokémon gacha
      const chosen = pickFromPool(EPIC_GACHA_POOL);
      const isShiny = Math.random() < 1 / 64;
      const level = Math.min(175, Math.max(1, Math.floor(gameState.wave * 0.3 + 5)));
      pk = newPokemonEntry(chosen.id, chosen.name, chosen.types, level, isShiny, true);
      pk.stats = await fetchPokemonStats(chosen.id);
      pk.statsLoaded = true;
      pk.currentHp = getMaxHp(pk);
      if(isCosmic(pk)) checkAndAnnounceCosmic(pk);
    ...
    gameState.box.push(pk);
    pulls.push(pk);
    if(pk._legend) addLog(`🌟 LIMITED LEGEND: ${pk.isShiny ? 'SHINY ' : ''}${pk.name} obtained!`, 'log-cosmic');
  }
  renderAll();

  if(count === 1) showGachaResult(pulls[0], { again: () => doLimitedLegendaryGacha(1), label: '🔁 Pull Again (700💎)' });
  else showGachaResultMulti(pulls, { again: () => doLimitedLegendaryGacha(10), label: '🔁 Pull 10× Again (6500💎)' });
}
