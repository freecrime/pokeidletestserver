...
const EPIC_ITEMS = [
  ...
  {id:'origin_core', name:'Origin Core',  emoji:'🌌', desc:'A core humming with raw spacetime. Held by Dialga or Palkia it unlocks Origin Forme — all stats doubled. With BOTH Origin formes on your team: lead with Dialga to slow the foe & rift-strike with Palkia, or lead with Palkia and Dialga hastes it to 1.5× attack speed.', effect:'origin_core'},
...
function isMegaRayquaza(pokemon) {
  ...
  if(pokemon._achievementMon) return false; // standalone achievement-reward Rayquaza never Mega Evolves
...
function isOriginDialga(pokemon) {
  if(!pokemon || pokemon.id !== 483) return false;
  return gameState.equippedItems[pokemon.uid] === 'origin_core';
}

function isOriginPalkia(pokemon) {
  if(!pokemon || pokemon.id !== 484) return false;
  return gameState.equippedItems[pokemon.uid] === 'origin_core';
}
...
function getSpriteUrl(id, isShiny=false, uid=null) {
  ...
  if((id === 483 || id === 484) && uid != null) {
    if(gameState && gameState.equippedItems && gameState.equippedItems[uid] === 'origin_core') {
      const oid = id === 483 ? 10245 : 10246; // Origin Dialga / Origin Palkia
      return isShiny
        ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/shiny/${oid}.png`
        : `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${oid}.png`;
    }
  }
  if(id === 646 && uid != null) {
