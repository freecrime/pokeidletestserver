...
// OPTIONS
// ============================================================
const gameOptions = { performance: false, animations: true, compact: false, ivNumbers: false, quietLog: false, hideDmgNumbers: false };
...
function syncOptionButtons() {
  ...
  setBtn('opt-quietlog-btn', gameOptions.quietLog, '#ffa726', '#ffa726');
  setBtn('opt-hidedmg-btn', gameOptions.hideDmgNumbers, '#ef5350', '#ef5350');
}
...
// Debug: force every Limited Legendary pull to be a random one of the 4 editions (for testing reveals).
window.debugGuaranteeLegendPull = false;
function debugToggleLegendPull() {
  window.debugGuaranteeLegendPull = !window.debugGuaranteeLegendPull;
  const btn = document.getElementById('dbg-legend-pull-btn');
  if(window.debugGuaranteeLegendPull) {
    if(btn) { btn.textContent = '🌟 Guarantee Legendary Pull: ON'; btn.style.background = 'rgba(255,200,0,0.35)'; btn.style.borderColor = '#ffd54f'; }
    toast('🌟 Limited Legendary pulls guaranteed!', 2000);
  } else {
    if(btn) { btn.textContent = '🌟 Guarantee Legendary Pull: OFF'; btn.style.background = 'rgba(255,200,0,0.12)'; }
    toast('Legendary pull guarantee removed.', 2000);
  }
}
...
//   • DIALGA  — normal raid, sits between Kyurem & Zygarde
//   • PALKIA  — normal raid, sits between Kyurem & Zygarde (a touch harder)
//   • ARCEUS  — CO-OP MULTIPLAYER raid (~2× Palkia), join-by-code
// ████████████████████████████████████████████████████████████

// ============================================================
// DIALGA BOSS  (Temporal Pokémon · Steel/Dragon · Lv.270)
//   HP 39,000 · atk×1.05 · cost 360💎   (Kyurem=37k/1.0, Zygarde=43k/0.9)
// ============================================================

let dialgaBattleActive = false;
let dialgaEnemy = null;

async function startDialgaBoss() {
  if(gameState.gems < 360) { toast('Need 360 💎 Gems to challenge Dialga!'); return; }
  if(gameState.team.length === 0) { toast('You need a team!'); return; }
  battlePaused = true;
  document.getElementById('auto-btn').textContent = '▶ RESUME';
  document.getElementById('modal-title').textContent = '⏳ DIALGA Lv.270';
  document.getElementById('modal-content').innerHTML = `
    <div style="text-align:center;padding:8px">
      <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/483.png" width="140" height="140" style="image-rendering:pixelated;filter:drop-shadow(0 0 24px rgba(80,170,230,0.9))">
      <div style="margin:10px 0;font-family:'Press Start 2P';font-size:8px;color:#74c0ec">THE TEMPORAL POKÉMON</div>
      <div style="font-size:14px;color:var(--text2);margin-bottom:12px">
        The ruler of time itself, roaring out of a fold in the timeline.<br>It hits harder than Kyurem.<br><br>
        <span style="color:#ffd700">Drop: Dialga (1/20 shiny!)</span><br>
        <span style="color:#74c0ec">39,000 HP · Bonus 💎 on victory</span><br>
        <span style="color:var(--text2)">Cost: 360 💎</span>
      </div>
      <div style="display:flex;gap:8px;justify-content:center">
        <button class="btn" style="border-color:#4da3e0;color:#74c0ec" onclick="confirmDialgaBoss()">⚔️ Fight! (360💎)</button>
        <button class="btn gold" onclick="closeModal()">Back</button>
      </div>
    </div>
  `;
...
async function confirmDialgaBoss(paid) {
  bossStarting = true;
  if(!paid) {
    if(gameState.gems < 360) { toast('Not enough Gems!'); closeModal(); bossStarting = false; return; }
    gameState.gems -= 360;
    updateResourceUI();
  }
  closeModal();
  ...
  const dStats = await fetchPokemonStats(483);
  dialgaEnemy = newPokemonEntry(483, 'Dialga', ['steel','dragon'], 270, false, true);
  dialgaEnemy.stats = dStats;
  dialgaEnemy.statsLoaded = true;
  dialgaEnemy._bossHpMax = 39000;
  dialgaEnemy.currentHp = dialgaEnemy._bossHpMax;
  dialgaEnemy.isBoss = true;
  dialgaEnemy._isDialga = true;
  dialgaEnemy._savedRoad = savedRoad;
  dialgaEnemy._attackMult = 1.05;
  dialgaEnemy._attackInterval = 1050;
  currentEnemy = dialgaEnemy;
  dialgaBattleActive = true;
  bossBattleActive = true;
  bossStarting = false;
  gameState.team.forEach(p => { if(p.statsLoaded) p.currentHp = getMaxHp(p); });
  document.getElementById('arena').style.background = 'radial-gradient(ellipse at 50% 0%, rgba(60,150,230,0.35) 0%, transparent 60%), radial-gradient(ellipse at 80% 90%, rgba(20,60,120,0.5) 0%, transparent 50%), linear-gradient(180deg, #00040f 0%, #001026 40%, #00060f 100%)';
  addLog('⏳ DIALGA tore open the flow of time!', 'log-cosmic');
  toast('⏳ BOSS FIGHT: Dialga Lv.270! Time itself trembles!', 4000);
  battlePaused = false;
  updateEnemyUI();
}

function handleDialgaDefeated(player) {
  const savedRoad = dialgaEnemy?._savedRoad;
  dialgaBattleActive = false;
  bossBattleActive = false;
  currentEnemy = null;
  dialgaEnemy = null;
  document.getElementById('arena').style.background = '';
  if(savedRoad && savedRoad.active) { gameState.road = savedRoad; document.getElementById('arena').classList.add('road-mode'); }
  addLog('🎉 DIALGA DEFEATED! Time bends to your will!', 'log-cosmic');
  toast('🏆 DIALGA CAPTURED!', 5000);
  saveGame();
  const gemReward = 30 + Math.floor(Math.random()*31);
  gameState.gems += gemReward;
  addLog(`💎 Dialga left behind ${gemReward} Gems!`, 'log-evolve');
  toast(`💎 +${gemReward} Gems!`, 3500);
  updateResourceUI();
  // 1% rare drop — 🌌 Origin Core (→ Origin Forme)
  if(debugGuaranteeRareDrop || Math.random() < 0.01) {
    gameState.inventory['origin_core'] = (gameState.inventory['origin_core'] || 0) + 1;
    addLog('🌌 The Origin Core tore loose from the flow of time — Dialga left it behind!', 'log-cosmic');
    toast('🌌 RARE DROP: Origin Core!', 5000);
  }
  const isShiny = debugGuaranteeShinyDrop || Math.random() < 1/20;
  const dlg = newPokemonEntry(483, 'Dialga', ['steel','dragon'], 270, isShiny);
  dlg.ivs = generateHighIVs();
  fetchPokemonStats(483).then(stats => {
    dlg.stats = stats;
    dlg.statsLoaded = true;
    dlg.currentHp = getMaxHp(dlg);
    gameState.box.push(dlg);
    checkAndAnnounceCosmic(dlg);
    renderAll();
    showGachaResult(dlg);
  });
  playerAttackCooldown = 0;
...
function handleDialgaFailed() {
  ...
  document.getElementById('arena').style.background = '';
  if(savedRoad && savedRoad.active) { gameState.road = savedRoad; document.getElementById('arena').classList.add('road-mode'); }
  addLog('💀 DIALGA rewound time and erased your team from the battle!', 'log-dmg');
  toast('💀 Dialga defeated your team!', 5000);
  saveGame();
  gameState.team.forEach(p => { p.currentHp = Math.max(1, Math.floor(getMaxHp(p)*0.20)); });
  gameState.currentFighterIdx = 0;
  playerAttackCooldown = 0;
...
// ============================================================
// PALKIA BOSS  (Spatial Pokémon · Water/Dragon · Lv.270)
//   HP 41,000 · atk×1.10 · cost 380💎
// ============================================================

let palkiaBattleActive = false;
let palkiaEnemy = null;

async function startPalkiaBoss() {
  if(gameState.gems < 380) { toast('Need 380 💎 Gems to challenge Palkia!'); return; }
  if(gameState.team.length === 0) { toast('You need a team!'); return; }
  battlePaused = true;
  document.getElementById('auto-btn').textContent = '▶ RESUME';
  document.getElementById('modal-title').textContent = '🌀 PALKIA Lv.270';
  document.getElementById('modal-content').innerHTML = `
    <div style="text-align:center;padding:8px">
      <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/484.png" width="140" height="140" style="image-rendering:pixelated;filter:drop-shadow(0 0 24px rgba(224,122,208,0.9))">
      <div style="margin:10px 0;font-family:'Press Start 2P';font-size:8px;color:#e79bd9">THE SPATIAL POKÉMON</div>
      <div style="font-size:14px;color:var(--text2);margin-bottom:12px">
        It warps the very dimensions of space to crush intruders.<br>Even fiercer than Dialga.<br><br>
        <span style="color:#ffd700">Drop: Palkia (1/20 shiny!)</span><br>
        <span style="color:#e79bd9">41,000 HP · Bonus 💎 on victory</span><br>
        <span style="color:var(--text2)">Cost: 380 💎</span>
      </div>
      <div style="display:flex;gap:8px;justify-content:center">
        <button class="btn" style="border-color:#d36bc2;color:#e79bd9" onclick="confirmPalkiaBoss()">⚔️ Fight! (380💎)</button>
        <button class="btn gold" onclick="closeModal()">Back</button>
      </div>
    </div>
  `;
...
async function confirmPalkiaBoss(paid) {
  bossStarting = true;
  if(!paid) {
    if(gameState.gems < 380) { toast('Not enough Gems!'); closeModal(); bossStarting = false; return; }
    gameState.gems -= 380;
    updateResourceUI();
  }
  closeModal();
  ...
  const pStats = await fetchPokemonStats(484);
  palkiaEnemy = newPokemonEntry(484, 'Palkia', ['water','dragon'], 270, false, true);
  palkiaEnemy.stats = pStats;
  palkiaEnemy.statsLoaded = true;
  palkiaEnemy._bossHpMax = 41000;
  palkiaEnemy.currentHp = palkiaEnemy._bossHpMax;
  palkiaEnemy.isBoss = true;
  palkiaEnemy._isPalkia = true;
  palkiaEnemy._savedRoad = savedRoad;
  palkiaEnemy._attackMult = 1.10;
  palkiaEnemy._attackInterval = 1050;
  currentEnemy = palkiaEnemy;
  palkiaBattleActive = true;
  bossBattleActive = true;
  bossStarting = false;
  gameState.team.forEach(p => { if(p.statsLoaded) p.currentHp = getMaxHp(p); });
  document.getElementById('arena').style.background = 'radial-gradient(ellipse at 50% 0%, rgba(210,110,200,0.35) 0%, transparent 60%), radial-gradient(ellipse at 80% 90%, rgba(110,30,120,0.5) 0%, transparent 50%), linear-gradient(180deg, #0a0010 0%, #1a0426 40%, #0d0010 100%)';
  addLog('🌀 PALKIA bent space and surged forward!', 'log-cosmic');
  toast('🌀 BOSS FIGHT: Palkia Lv.270! Space distorts around you!', 4000);
  battlePaused = false;
  updateEnemyUI();
}

function handlePalkiaDefeated(player) {
  const savedRoad = palkiaEnemy?._savedRoad;
  palkiaBattleActive = false;
  bossBattleActive = false;
  currentEnemy = null;
  palkiaEnemy = null;
  document.getElementById('arena').style.background = '';
  if(savedRoad && savedRoad.active) { gameState.road = savedRoad; document.getElementById('arena').classList.add('road-mode'); }
  addLog('🎉 PALKIA DEFEATED! Space settles in your favor!', 'log-cosmic');
  toast('🏆 PALKIA CAPTURED!', 5000);
  saveGame();
  const gemReward = 35 + Math.floor(Math.random()*36);
  gameState.gems += gemReward;
  addLog(`💎 Palkia left behind ${gemReward} Gems!`, 'log-evolve');
  toast(`💎 +${gemReward} Gems!`, 3500);
  updateResourceUI();
  // 1% rare drop — 🌌 Origin Core (→ Origin Forme)
  if(debugGuaranteeRareDrop || Math.random() < 0.01) {
    gameState.inventory['origin_core'] = (gameState.inventory['origin_core'] || 0) + 1;
    addLog('🌌 The Origin Core tore loose from warped space — Palkia left it behind!', 'log-cosmic');
    toast('🌌 RARE DROP: Origin Core!', 5000);
  }
  const isShiny = debugGuaranteeShinyDrop || Math.random() < 1/20;
  const plk = newPokemonEntry(484, 'Palkia', ['water','dragon'], 270, isShiny);
  plk.ivs = generateHighIVs();
  fetchPokemonStats(484).then(stats => {
    plk.stats = stats;
    plk.statsLoaded = true;
    plk.currentHp = getMaxHp(plk);
    gameState.box.push(plk);
    checkAndAnnounceCosmic(plk);
    renderAll();
    showGachaResult(plk);
  });
  playerAttackCooldown = 0;
...
function handlePalkiaFailed() {
  ...
  document.getElementById('arena').style.background = '';
  if(savedRoad && savedRoad.active) { gameState.road = savedRoad; document.getElementById('arena').classList.add('road-mode'); }
  addLog('💀 PALKIA folded space and swept your whole team aside!', 'log-dmg');
  toast('💀 Palkia defeated your team!', 5000);
  saveGame();
  gameState.team.forEach(p => { p.currentHp = Math.max(1, Math.floor(getMaxHp(p)*0.20)); });
  gameState.currentFighterIdx = 0;
  playerAttackCooldown = 0;
...
// ============================================================
// DIALGA & PALKIA — MERGED RAID  (Creation Duo · 50/50 · one entry)
//   One challenge, random which titan appears. Unified cost 500💎.
//   1% drop: 🌌 Origin Core → Origin Forme (2× stats) for either.
// ============================================================

async function startDialgaPalkiaBoss() {
  const COST = 500;
  if(gameState.gems < COST) { toast(`Need ${COST} 💎 Gems to challenge the Creation Duo!`); return; }
  if(gameState.team.length === 0) { toast('You need a team!'); return; }
  battlePaused = true;
  document.getElementById('auto-btn').textContent = '▶ RESUME';
  const isDialga = Math.random() < 0.5;          // 50/50 which one you face
  const id = isDialga ? 483 : 484;
  const name = isDialga ? 'Dialga' : 'Palkia';
  const color = isDialga ? '#74c0ec' : '#e79bd9';
  const glow = isDialga ? 'rgba(80,170,230,0.9)' : 'rgba(224,122,208,0.9)';
  const subtitle = isDialga ? '⏳ THE TEMPORAL POKÉMON' : '🌀 THE SPATIAL POKÉMON';
  const blurb = isDialga
    ? 'The ruler of time roars out of a fold in the timeline.'
    : 'The ruler of space warps dimensions to crush intruders.';
  document.getElementById('modal-title').textContent = '⏳🌀 THE CREATION DUO';
  document.getElementById('modal-content').innerHTML = `
    <div style="text-align:center;padding:8px">
      <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${id}.png" width="140" height="140" style="image-rendering:pixelated;filter:drop-shadow(0 0 24px ${glow})">
      <div style="margin:10px 0;font-family:'Press Start 2P';font-size:8px;color:${color}">${subtitle}</div>
      <div style="font-size:14px;color:var(--text2);margin-bottom:12px">
        ${blurb}<br>Dialga or Palkia appears at random — 50/50!<br><br>
        <span style="color:#ffd700">Drop: ${name} (1/20 shiny!)</span><br>
        <span style="color:${color}">Lv.270 · Bonus 💎 on victory</span><br>
        <span style="color:#ffd700;font-size:12px">1% 🌌 Origin Core → Origin Forme · 2× stats!</span><br>
        <span style="color:var(--text2)">Cost: ${COST} 💎</span>
      </div>
      <div style="display:flex;gap:8px;justify-content:center">
        <button class="btn" style="border-color:${color};color:${color}" onclick="confirmDialgaPalkiaBoss(${id})">⚔️ Fight! (${COST}💎)</button>
        <button class="btn gold" onclick="closeModal()">Back</button>
      </div>
    </div>
  `;
...
async function confirmDialgaPalkiaBoss(pokemonId) {
  const COST = 500;
  if(gameState.gems < COST) { toast('Not enough Gems!'); closeModal(); return; }
  gameState.gems -= COST;
  updateResourceUI();
  // Hand off to the existing per-boss setup (cost already paid)
  if(pokemonId === 483) confirmDialgaBoss(true);
  else confirmPalkiaBoss(true);
}

// ============================================================
// ARCEUS BOSS  (The Original One · Normal · Lv.290)
//   A normal solo raid like the others (toned down from the old
//   co-op version). HP 52,000 · atk×1.05 · cost 1500💎
// ============================================================

const ARCEUS_RAID = { id: 493, name: 'Arceus', types: ['normal'], level: 290, hpMax: 52000, attackMult: 1.05 };

let arceusRaidActive = false;
let arceusEnemy = null;

async function startArceusBoss() {
  if(gameState.gems < 1500) { toast('Need 1500 💎 Gems to challenge Arceus!'); return; }
  if(gameState.team.length === 0) { toast('You need a team!'); return; }
  battlePaused = true;
  document.getElementById('auto-btn').textContent = '▶ RESUME';
  document.getElementById('modal-title').textContent = '✨ ARCEUS Lv.290';
  document.getElementById('modal-content').innerHTML = `
    <div style="text-align:center;padding:8px">
      <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/493.png" width="140" height="140" style="image-rendering:pixelated;filter:drop-shadow(0 0 26px rgba(255,225,150,0.95))">
      <div style="margin:10px 0;font-family:'Press Start 2P';font-size:8px;color:#ffe7a3">THE ORIGINAL ONE</div>
      <div style="font-size:14px;color:var(--text2);margin-bottom:12px">
        The Pokémon said to have shaped the universe.<br>The toughest raid on the road.<br><br>
        <span style="color:#ffd700">Drop: Arceus (1/15 shiny!)</span><br>
        <span style="color:#ffe7a3">52,000 HP · Bonus 💎 on victory</span><br>
        <span style="color:var(--text2)">Cost: 1500 💎</span>
      </div>
      <div style="display:flex;gap:8px;justify-content:center">
        <button class="btn" style="border-color:#ffd54f;color:#ffe7a3" onclick="confirmArceusBoss()">⚔️ Fight! (1500💎)</button>
        <button class="btn gold" onclick="closeModal()">Back</button>
      </div>
    </div>
  `;
...
async function confirmArceusBoss() {
  bossStarting = true;
  if(gameState.gems < 1500) { toast('Not enough Gems!'); closeModal(); bossStarting = false; return; }
  gameState.gems -= 1500;
  updateResourceUI();
  closeModal();
  battlePaused = true;
  ...
  const aStats = await fetchPokemonStats(ARCEUS_RAID.id);
  arceusEnemy = newPokemonEntry(ARCEUS_RAID.id, ARCEUS_RAID.name, ARCEUS_RAID.types, ARCEUS_RAID.level, false, true);
  arceusEnemy.stats = aStats;
  arceusEnemy.statsLoaded = true;
  arceusEnemy._bossHpMax = ARCEUS_RAID.hpMax;
  arceusEnemy.currentHp = arceusEnemy._bossHpMax;
  arceusEnemy.isBoss = true;
  arceusEnemy._isArceus = true;
  arceusEnemy._savedRoad = savedRoad;
  arceusEnemy._attackMult = ARCEUS_RAID.attackMult;
  arceusEnemy._attackInterval = 900;   // fast (god-tier), switch-grace still lets your team act
  currentEnemy = arceusEnemy;
  arceusRaidActive = true;
  bossBattleActive = true;
  bossStarting = false;
  gameState.team.forEach(p => { if(p.statsLoaded) p.currentHp = getMaxHp(p); });
  document.getElementById('arena').style.background = 'radial-gradient(ellipse at 50% 0%, rgba(255,225,150,0.30) 0%, transparent 60%), radial-gradient(ellipse at 50% 100%, rgba(180,140,40,0.45) 0%, transparent 55%), linear-gradient(180deg, #0a0700 0%, #1a1400 45%, #0a0700 100%)';
  addLog('✨ ARCEUS descended — the Original One judges you!', 'log-cosmic');
  toast('✨ BOSS FIGHT: Arceus Lv.290!', 4000);
  battlePaused = false;
  updateEnemyUI();
}

function handleArceusDefeated(player) {
  const savedRoad = arceusEnemy?._savedRoad;
  arceusRaidActive = false;
  bossBattleActive = false;
  currentEnemy = null;
  arceusEnemy = null;
  document.getElementById('arena').style.background = '';
  if(savedRoad && savedRoad.active) { gameState.road = savedRoad; document.getElementById('arena').classList.add('road-mode'); }
  addLog('🎉 ARCEUS DEFEATED! The Original One acknowledges you!', 'log-cosmic');
  toast('🏆 ARCEUS CAPTURED!', 5000);
  saveGame();
  const gemReward = 80 + Math.floor(Math.random()*61);
  gameState.gems += gemReward;
  addLog(`💎 Arceus left behind ${gemReward} Gems!`, 'log-evolve');
  toast(`💎 +${gemReward} Gems!`, 3500);
  updateResourceUI();
  const isShiny = debugGuaranteeShinyDrop || Math.random() < 1/15;
  const arc = newPokemonEntry(ARCEUS_RAID.id, ARCEUS_RAID.name, ARCEUS_RAID.types, ARCEUS_RAID.level, isShiny);
  arc.ivs = generateHighIVs();
  fetchPokemonStats(ARCEUS_RAID.id).then(stats => {
    arc.stats = stats;
    arc.statsLoaded = true;
    arc.currentHp = getMaxHp(arc);
    gameState.box.push(arc);
    checkAndAnnounceCosmic(arc);
    renderAll();
    showGachaResult(arc);
  });
  playerAttackCooldown = 0;
...
function handleArceusFailed() {
  ...
  document.getElementById('arena').style.background = '';
  if(savedRoad && savedRoad.active) { gameState.road = savedRoad; document.getElementById('arena').classList.add('road-mode'); }
  addLog('💀 ARCEUS passed judgement — your team was wiped out!', 'log-dmg');
  toast('💀 Arceus defeated your team!', 5000);
  saveGame();
  gameState.team.forEach(p => { p.currentHp = Math.max(1, Math.floor(getMaxHp(p)*0.20)); });
  gameState.currentFighterIdx = 0;
  playerAttackCooldown = 0;
...
// ============================================================
// OPTIONS — persistence + extra toggles
// ============================================================
function loadGameOptions() {
  try {
    const raw = localStorage.getItem('pkm_options');
    if(raw) {
      const saved = JSON.parse(raw);
      Object.keys(saved).forEach(k => { if(k in gameOptions) gameOptions[k] = saved[k]; });
    }
  } catch(e) {}
  window._gameOptions = gameOptions;
  applyOptions();
  syncOptionButtons();
}
function saveGameOptions() {
  try { localStorage.setItem('pkm_options', JSON.stringify(gameOptions)); } catch(e) {}
}
// Load persisted options once everything is defined.
loadGameOptions();
